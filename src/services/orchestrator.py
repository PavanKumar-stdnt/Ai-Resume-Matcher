...
class MatchOrchestrator:
    ...
    def run(self, resume_bytes: bytes, resume_filename: str, job_text: str) -> dict[str, Any]:
        state = MatchState(
        ...
        logger.info("Pipeline END — final_score=%.2f, elapsed=%.2fs", state.final_score, state.elapsed_seconds)
        return self._build_result(state)

    # ── Batch run ─────────────────────────────────────────────────────────────

    def run_batch(self, resumes: list[dict[str, Any]], job_text: str) -> list[dict[str, Any]]:
        if not resumes:
        ...
        for idx, resume in enumerate(resumes, start=1):
            ...
            if not raw_bytes:
                errors.append({"resume_filename": filename, "error": "Empty file.", "final_score": 0.0})
                continue
            logger.info("[Batch %d/%d] Processing '%s' …", idx, len(resumes), filename)
            try:
                results.append(self.run(resume_bytes=raw_bytes, resume_filename=filename, job_text=cleaned_jd))
            except Exception as exc:
                logger.error("[Batch] Failed for '%s': %s", filename, exc)
                errors.append({"resume_filename": filename, "error": str(exc), "final_score": 0.0,
                                "vector_score": 0.0, "llm_score": 0.0, "analysis": "", "warnings": [str(exc)]})
    ...
    def _stage1_parse(self, state: MatchState) -> None:
        state.resume_text = extract_text_from_bytes(state.resume_bytes, state.resume_filename)
        state.stage1_complete = True

    def _stage2a_index(self, state: MatchState) -> None:
        point_ids = self._vector.index_resume(state.resume_text, state.resume_filename, state.resume_id)
        state.chunks_indexed = len(point_ids)
        state.stage2a_complete = True

    def _stage2b_retrieve(self, state: MatchState) -> None:
        try:
            state.retrieved_chunks = self._vector.retrieve_top_k(state.job_text, state.resume_id)
            state.vector_score = self._vector.compute_vector_score(state.job_text, state.resume_id)
    ...
    def _compute_final_score(state: MatchState) -> None:
        state.final_score = round(state.vector_score * _VECTOR_WEIGHT + state.llm_score * _LLM_WEIGHT, 2)
